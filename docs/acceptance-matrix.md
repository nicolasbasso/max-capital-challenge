...
| R15 | Al completarse una orden (`status = FILLED`) se publica un mensaje de `settlement` a un destino downstream, de modo que downstream lo reciba **exactamente una vez por orden**, aun con reentregas o con las dos instancias viendo el completado. | **Una orden produce a lo sumo un efecto de liquidación downstream; y produce uno salvo que su completado quede contradicho antes de haber podido informarlo.** El "a lo sumo uno" es del efecto, no del mensaje: la entrega es al menos una vez y la deduplicación es de downstream, por `numericOrderId` más el tipo. | Con las dos instancias corriendo: se completa una orden, se reentrega el ER que la completó, y se corre el ciclo entero varias veces. Por separado, una orden que completa y recibe un ER tardío antes de que el barrido pase. | `SettlementTest`: ocho casos, entre ellos la reentrega del ER que completó y el completado contradicho. En docker con dos instancias, un solo `ORDER_SETTLED` por orden y publicado por **una sola** de las dos, con la otra en cero. |
| R16 | Las órdenes que terminan en `CANCELLED` **no** emiten settlement. | **Una orden que no completó no produce ningún efecto de liquidación.** No es una regla aparte: la consulta que decide qué publicar sólo mira las órdenes completadas, así que cancelada e incompleta quedan afuera por construcción. | Se cancela una orden después de un fill parcial y se corren los dos barridos. Se deja además una orden `INCOMPLETE` que nunca completó. | `SettlementTest`: `unaOrdenCanceladaNoProduceSettlement` y `unaOrdenIncompletaQueNuncaCompletoNoProduceNada`. Ambos se ponen en rojo si se quita el filtro por estado del barrido. |
